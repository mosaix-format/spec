---
title: Conventions
id: 01JVKE4X2PTRNG6SM4BXVFW73Y
type: meta
updated: 2026-09-05
tags: [1.1, checker, commitment, composed-document, composition, conformance, conventions, core, document, folders, frontmatter, graph, guide, history, host, key, keywords, ledger, license, links, markdown, meta, moc, mosaix, note, obsidian, onboarding, proposal, provenance, purpose, rule, rules, scope, spec, staleness, status, structure, substrate, summary, superseded, synthesis, taxonomy, template, time, verifiable, versioning]
status: sourced
summary: "Meta note of this vault: folder contract, reliability convention (status key), declared tags and entity types, target version mosaix 1.1, and who maintains it."
keywords: [conventions, meta note, folder contract, reliability convention, declared tags, status, maintainer, mosaix 1.1]
entities: []
relations: []
links: [§5 Vault structure, R4 Declared taxonomy, Open questions]
mosaix: "1.1"
rev: 99ccaaddbbee
---

# Conventions

mosaix: "1.1". Folders: `spec/` sections, `keys/` core keys, `rules/`, `proposals/` for 1.1, `_meta/`, `_synthesis/`, `_docs/` composed documents, `templates/` a starter template for each note type. Reliability: frontmatter key `status` with values sourced · to-confirm · superseded. Tags declared: the `tags` list of this note is the taxonomy of the vault (46 tags). Entity types: the eight of §3. .
