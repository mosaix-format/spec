---
title: Opening the second shop — brief
id: 01JVTQ8YBN5PNHGXDBTRYMWKFV
type: document
updated: 2026-09-05
tags: [composed-document]
status: sourced
summary: "Composed document for the partners to review before deciding on a second shop: the neighbourhood, the Saturday market, delivery policy, Monday closure and organic certification, with two notes excluded."
keywords: [second shop, expansion brief, composed document, partner decision, five fragments, neighbourhood analysis, growth plan]
entities:
  - {name: Forno Vialetto, type: company}
relations: []
links: [Viale Giardini neighbourhood, Saturday market at Piazza Solari, No delivery policy, Closed on Mondays, Organic certification]
fragments: [Viale Giardini neighbourhood, Saturday market at Piazza Solari, No delivery policy, Closed on Mondays, Organic certification]
pool: [Café Lindström, Panificio Reale]
layout:
  cover: {title: Opening the second shop, subtitle: Partners' reading, date: 2026-09-05}
  page: A4
  sections:
    - {title: Context, fragments: [Viale Giardini neighbourhood, Saturday market at Piazza Solari]}
    - {title: Constraints, fragments: [No delivery policy, Closed on Mondays]}
    - {title: Open question, fragments: [Organic certification]}
  show: [summary, status]
rev: 5a6b7c8d9e0f
---

# Opening the second shop — brief

Read the five fragments in order. This document assembles what the partners need to review before deciding on a second location.

![[Viale Giardini neighbourhood]]
![[Saturday market at Piazza Solari]]
![[No delivery policy]]
![[Closed on Mondays]]
![[Organic certification]]

Not included: Café Lindström and Panificio Reale (competitor profiles, in the pool for reference).
