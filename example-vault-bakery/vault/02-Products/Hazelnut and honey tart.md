---
title: Hazelnut and honey tart
id: 01JVTQ8YBNGRXMPKFNBDWHVTYQ
type: product
updated: 2026-09-05
tags: [product, pastry, seasonal]
status: sourced
summary: "Revised hazelnut tart replacing cane sugar with chestnut honey and adding a pinch of sea salt; introduced January 2024 after the original was judged too sweet by regular customers."
keywords: [hazelnut honey tart, revised recipe, chestnut honey, sea salt, less sweet, replaced version, customer feedback]
entities:
  - {name: Hazelnut and honey tart, type: product}
  - {name: Hazelnut and honey tart recipe, type: recipe}
relations: []
links: [Hazelnut tart (old recipe), Layla Beshir]
rev: 1e2f3a4b5c6d
---

# Hazelnut and honey tart

Revised version of the [[Hazelnut tart (old recipe)]], introduced in January 2024. Replaces cane sugar with chestnut honey (from a local apiary) and adds a pinch of sea salt to balance the sweetness. Developed by [[Layla Beshir]] after three months of customer feedback. ✅ sourced (recipe log, 2024-01).

Retail price: €4.50 per slice. Available autumn and winter only (honey supply is seasonal). Sells about 15 slices per day in season.
